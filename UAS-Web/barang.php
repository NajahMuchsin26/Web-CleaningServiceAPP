      ...
      <div class="col-lg-12">
        ...
        <div class="card">
          <div class="card-body">
            ...
            <a href="crud/formbarang.php"><button class="btn btn-secondary">add</button></a>
            ...
            <button class="btn btn-primary btn-sm me-1 p-2 btn-data" data-status="delete_barang" onclick="checked(this)"
                ...
                <tbody>
                  ...
                  if ($result && mysqli_num_rows($result) > 0) {
                    ...
                    while ($data = mysqli_fetch_assoc($result)) {
                      ?>
                      <tr>
                        <td><input type="checkbox" class="mt-1 checkbox" value="<?php echo $data['id_barang'] ?>">
                        <td>
                          <?php echo $no ?>
                        </td>
                        <td>
                        ...
                        <td class="d-flex">
                          <button class="btn btn-warning btn-sm me-2 p-2 btn-data"
                            data-index="<?php echo $data['id_barang'] ?>" data-status="edit_barang" data-bs-toggle="modal"
                            data-bs-target="#modalExample"><i class="bi bi-pencil-square"></i></button>
                          <button class="btn btn-danger btn-sm me-1 p-2 btn-data"
                            data-index="<?php echo $data['id_barang'] ?>" data-status="delete_barang" data-bs-toggle="modal"
  ...
  <script>
    ...
    buttonsData.forEach((btnData) => {
      btnData.addEventListener('click', function () {
        ...
        const stokBarang = btnData.parentElement.parentElement.querySelector('td:nth-child(5)').innerText;

        const statusData = btnData.dataset.status;

        if (statusData === 'edit_barang') {
          xhr.onreadystatechange = function () {
            ...
            <div class="modal-body d-flex flex-column gap-3">
              <div class="d-flex flex-column gap-2">
                ...
                <input type="text" id="stok_barang" name="stok_barang" required value="${response['stok_barang']}">
              </div>
          ...
          xhr.open('POST', `./crud/api.php?id_barang=${indexData}&stok_barang=${stokBarang}&action=${statusData}`, true);
          xhr.send();
        } else if (statusData === 'delete_barang') {
    ...
    function editData(indexData) {
      ...
      form.addEventListener('submit', function () {
        ...
        xhr.open('POST', `./crud/api.php?id_barang=${indexData}&stok_barang=${stokBarang.value}&action=submit_edit_barang`, true);
        xhr.send();
      })
    }
    ...
    function deleteData(indexData) {
      ...
      btnDeleteData.addEventListener('click', function () {
        ...
        xhr.open('POST', `./crud/api.php?id_barang=${[indexData]}&action=submit_delete_barang`, true);
        xhr.send();
      })
    }
      ...
      btnDeleteData.addEventListener('click', function () {
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            console.log(response);
            if (response === 'success') {
              alert('Semua data barang berhasil dihapus, kecuali foreign_key_column!');
            } else {
              alert('Semua data barang gagal dihapus!');
            }
        ...
        xhr.open('POST', `./crud/api.php?action=delete_all_barang`, true);
        xhr.send();
      })
    }
  </script>
