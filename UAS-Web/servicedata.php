      ...
      <div class="col-lg-12">
        ...
        <div class="card">
          <div class="card-body">
            ...
            <a href="crud/formservice.php"><button class="btn btn-secondary">add</button></a>
            ...
            <button class="btn btn-primary btn-sm me-1 p-2 btn-data" data-status="delete_service"
                ...
                <tbody>
                  ...
                  if ($result && mysqli_num_rows($result) > 0) {
                    ...
                    while ($data = mysqli_fetch_assoc($result)) {
                      ?>
                      <tr>
                        <td><input type='checkbox' class="checkbox" value="<?php echo $data['id_service'] ?>"></td>
                        <td>
                          <?php echo $data['id_service'] ?>
                        </td>
                        <td>
                          <?php echo $data['jenis_service'] ?>
                        </td>
                        <td>
                          <?php echo $data['harga'] ?>
                        </td>
                        <td class="d-flex">
                          <button class="btn btn-warning btn-sm me-2 p-2 btn-data"
                            data-index="<?php echo $data['id_service'] ?>" data-status="edit_service" data-bs-toggle="modal"
                            data-bs-target="#modalExample"><i class="bi bi-pencil-square"></i></button>
                          <button class="btn btn-danger btn-sm me-1 p-2 btn-data"
                            data-index="<?php echo $data['id_service'] ?>" data-status="delete_service"
  ...
  <script>
    ...
    buttonsData.forEach((btnData) => {
      btnData.addEventListener('click', function () {
        ...
        const jenisService = btnData.parentElement.parentElement.querySelector('td:nth-child(3)').innerText;
        const harga = btnData.parentElement.parentElement.querySelector('td:nth-child(4)').innerText;
        const statusData = btnData.dataset.status;

        if (statusData === 'edit_service') {
          xhr.onreadystatechange = function () {
            if (xhr.status === 200 && xhr.readyState === 4) {
              const response = JSON.parse(xhr.responseText);

              modalTitle.textContent = 'Edit Data Service';
              modalContent.innerHTML = `<form method="POST" class="form">
            <div class="modal-body d-flex flex-column gap-3">
              <div class="d-flex flex-column gap-2">
                <label for="jenis_service">Jenis Service</label>
                <input type="text" id="jenis_service" name="jenis_service" required value="${response['jenis_service']}">
              </div>
              <div class="d-flex flex-column gap-2">
                <label for="harga">Harga</label>
                <input type="text" id="harga" name="harga" required value="${response['harga']}">
              </div>
          ...
          xhr.open('POST', `./crud/api.php?id_service=${indexData}&jenis_service=${jenisService}&harga=${harga}&action=${statusData}`, true);
          xhr.send();
        } else if (statusData === 'delete_service') {
    ...
    function editData(indexData) {
      const jenisService = document.getElementById('jenis_service');
      ...
      form.addEventListener('submit', function () {
        xhr.onreadystatechange = function (e) {
          ...
          if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (response === 'success') {
              alert('Data service berhasil diubah!');
            } else {
              alert('Data service gagal diubah!');
            }
          }
        }

        xhr.open('POST', `./crud/api.php?id_service=${indexData}&jenis_service=${jenisService.value}&harga=${harga.value}&action=submit_edit_service`, true);
        xhr.send();
      })
    }
    ...
    function deleteData(indexData) {
      ...
      btnDeleteData.addEventListener('click', function () {
        ...
        xhr.open('POST', `./crud/api.php?id_service=${[indexData]}&action=submit_delete_service`, true);
        xhr.send();
      })
    }
      ...
      btnDeleteData.addEventListener('click', function () {
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            console.log(response);
            if (response === 'success') {
              alert('Semua data service berhasil dihapus, kecuali foreign_key_column!');
            } else {
              alert('Semua data service gagal dihapus!');
            }
        ...
        xhr.open('POST', `./crud/api.php?action=delete_all_service`, true);
        xhr.send();
      })
    }
  </script>
