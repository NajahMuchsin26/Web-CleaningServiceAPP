...
if (isset($_POST['submit'])) {
  ...
  $id_barang = explode('~', $_POST['id_barang'])[0];
  $stok_barang = $_POST['stok_barang'];
  $request_stok_barang = $_POST['request_stok_barang'];
  ...
  if ($id_barang != 0) {
    if ($stok_barang > $request_stok_barang) {
      ...
      $stokBarangTerbaru = $stok_barang - $request_stok_barang;
      $sqlUpdateStok = "UPDATE inventory SET stok_barang = $stokBarangTerbaru WHERE id_barang = $id_barang";
      $resultUpdateStok = $conn->query($sqlUpdateStok);

      if (mysqli_affected_rows($conn) > 0) {
        echo "<script>alert('Pembuatan schedule berhasil!'); window.location.href = 'schedule.php'</script>";
      } else {
        echo "Error: " . $sql . "<br>" . mysqli_error($conn);
      }
    } else {
      echo "<script>alert('Permintaan barang tidak boleh lebih banyak dari stok barang!');</script>";
    }
  } else {
    echo "<script>alert('Pilih terlebih dahulu nama barang!');</script>";
  }
...
<body>
        ...
        <div>
          <div class="card">
            <div class="card-body">
              ...
              <form method="POST">
                <div class="row mb-3">
                  <label for="inputText" class="col-sm-2 col-form-label">Date Schedule</label>
                  <div class="col-sm-10">
                    <input type="date" class="form-control" required name="tanggal_Schedule">
                  </div>
                </div>
                <div class="row mb-3">
                  <label for="inputTime" class="col-sm-2 col-form-label">Time Start</label>
                  <div class="col-sm-10">
                    <input type="time" class="form-control" step="any" name="waktu_mulai">
                  </div>
                </div>
                <div class="row mb-3">
                  <label for="inputTime" class="col-sm-2 col-form-label">Time Finish</label>
                  <div class="col-sm-10">
                    <input type="time" class="form-control" required step="any" name="waktu_selesai">
                  </div>
                </div>
                <div class="row mb-3">
                  <label for="id_cleaner" class="col-sm-2 col-form-label">Cleaner</label>
                  <div class="col-sm-10">
                    <select name="id_cleaner" id="id_cleaner" required>
                  ...
                  <div class="col-sm-10">
                    <select name="id_barang" id="id_barang" required>
                      ...
                      <?php while ($dataBarang = mysqli_fetch_assoc($result_inventory)) {
                        ?>
                        <option value="<?php echo $dataBarang['id_barang'] ?>~<?php echo $dataBarang['stok_barang'] ?>">
                ...
                <div class="row mb-3">
                  <label for="stok_barang" class="col-sm-2 col-form-label">Sisa Stok Barang</label>
                  <div class="col-sm-10">
                    <input type="text" readonly id="stok_barang" name="stok_barang" class="form-control" value="0">
                  </div>
                </div>
                <div class="row mb-3">
                  <label for="request_stok_barang" class="col-sm-2 col-form-label">Permintaan Stok Barang</label>
                  <div class="col-sm-10">
                    <input type="text" id="request_stok_barang" name="request_stok_barang" class="form-control"
                      required>
                  </div>
                </div>
  ...
  <script>
    const namaBarang = document.getElementById('id_barang');
    const inputStockBarang = document.getElementById('stok_barang');
    namaBarang.addEventListener('change', function () {
      const sisaStokBarang = namaBarang.value.split('~')[1];
      inputStockBarang.value = sisaStokBarang;
    })
  </script>
</body>
