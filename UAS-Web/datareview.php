                ...
                <tbody>
                  ...
                  if ($result && mysqli_num_rows($result) > 0) {
                    ...
                    while ($data = mysqli_fetch_assoc($result)) {
                      ?>
                      <tr>
                        <td><input type="checkbox" class="mt-1 checkbox" value="<?php echo $data['id_review'] ?>">
                        <td>
                          <?php echo $no ?>
                        </td>
                        <td>
                          <?php echo $data['id_review'] ?>
                        </td>
                        <td>
                          <?php echo $data['isi_review'] ?>
                        </td>
                        <td>
                          <?php echo $data['waktu_review'] ?>
                        </td>
                        <td>
                          <?php echo $data['id_customer'] ?>
                        </td>
                        <td>
                          <?php echo $data['id_payment'] ?>
                        </td>
                        <td>
                        ...
                        <td class="d-flex">
                          <button class="btn btn-danger btn-sm me-1 p-2 btn-data"
                            data-index="<?php echo $data['id_review'] ?>" data-status="delete_review" data-bs-toggle="modal"
                            data-bs-target="#modalExample" data-status="delete_review"><i class="bi bi-trash3"></i></button>
                        </td>
                      </tr>
      ...
      <div class="modal-content">
        ...
        <div class="modal-content-custom">
          <div class="modal-footer justify-content-center">
            ...
            <button type="button" name="submit_data" class="btn btn-danger" data-id="">Ya, saya
              yakin</button>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    function deleteData(indexData) {
      ...
      btnDeleteData.addEventListener('click', function () {
        ...
        xhr.open('POST', `./crud/api.php?id_review=${[indexData]}&action=submit_delete_review`, true);
        xhr.send();
      })
    }
      ...
      btnDeleteData.addEventListener('click', function () {
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            console.log(response);
            if (response === 'success') {
              alert('Semua data review berhasil dihapus!');
            } else {
              alert('Semua data review gagal dihapus!');
            }
        ...
        xhr.open('POST', `./crud/api.php?action=delete_all_review`, true);
        xhr.send();
      })
    }
  </script>
