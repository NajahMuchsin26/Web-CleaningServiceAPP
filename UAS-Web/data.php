...
$query = "SELECT orders.*, register.username AS username, service.jenis_service AS jenis_service 
FROM orders 
...
<body>
      ...
      <div class="col-lg-12">
        ...
        <div class="card">
          <div class="card-body">
            ...
            <button class="btn btn-warning btn-sm me-1 p-2 btn-data" data-status="edit_status_order"
              onclick="checked(this)" data-bs-toggle="modal" data-bs-target="#modalExample">Ubah Status</button>
                ...
                <tbody>
                  ...
                  if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($data = mysqli_fetch_assoc($result)) {
                      ?>
                      <tr>
                        <td><input type="checkbox" class="mt-1 checkbox" value="<?php echo $data['id_order'] ?>">
                        </td>
                        <td>
                          <?php echo $no ?>
                        </td>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Yakin ingin mengubah status data sekaligus?</h5>
        ...
        <div class="modal-content-custom">
          <div class="modal-footer justify-content-center">
            ...
            <button type="button" name="submit_data" class="btn btn-primary">Ya, saya yakin</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    const editStatusData = document.querySelector('.btn-data');
    editStatusData.addEventListener('click', function () {
      const xhr = new XMLHttpRequest();
      const idOrder = editStatusData.dataset.index;
      const btnSubmitEdit = document.querySelector('[name="submit_data"]');
      ...
      btnSubmitEdit.addEventListener('click', function () {
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (response === 'success') {
              alert('Semua status order berhasil diubah!');
              window.location.reload();
            } else {
              alert('Semua status order gagal diubah!');
              window.location.reload();
            }
          }
        }

        xhr.open('POST', `crud/api.php?action=edit_status_order&id_order=${idOrder}`);
        xhr.send();
      })
    })
  </script>
