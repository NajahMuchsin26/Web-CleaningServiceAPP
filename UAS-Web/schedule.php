...
$query = "SELECT *
FROM schedule
LEFT JOIN register ON schedule.id_cleaner = register.id_user
WHERE register.id_role = 3 ORDER BY id_schedule DESC";
...
<head>
  ...
  <title> Smart Clean :: DataAdmin</title>
      ...
      <div class="col-lg-12">
        ...
        <div class="card">
          <div class="card-body">
            ...
            <a href="schedule1.php"><button class="btn btn-secondary">add</button></a>
            ...
            <button class="btn btn-primary btn-sm me-1 p-2 btn-data" data-status="delete_schedule"
              onclick="checked(this)" data-bs-toggle="modal" data-bs-target="#modalExample"><i
                ...
                <tbody>
                  ...
                  if ($result && mysqli_num_rows($result) > 0) {
                    ...
                    while ($data = mysqli_fetch_assoc($result)) {
                      ?>
                      <tr>
                        <td><input type="checkbox" class="mt-1 checkbox" value="<?php echo $data['id_schedule'] ?>">
                        </td>
                        <td>
                          <?php echo $no; ?>
                        </td>
                        <td>
                          <?php echo $data['tanggal_schedule'] ?>
                        </td>
                        <td>
                          <?php echo $data['waktu_mulai'] ?>
                        </td>
                        <td>
                          <?php echo $data['waktu_selesai'] ?>
                        </td>
                        <td data-id-cleaner="<?php echo $data['id_cleaner'] ?>">
                          <?php echo $data['username'] ?>
                        </td>
                        <td class="d-flex">
                          <button class="btn btn-warning btn-sm me-2 p-2 btn-data"
                            data-index="<?php echo $data['id_schedule'] ?>" data-status="edit_schedule"
                            data-bs-toggle="modal" data-bs-target="#modalExample"><i
                              class="bi bi-pencil-square"></i></button>
                          <button class="btn btn-danger btn-sm me-1 p-2 btn-data"
                            data-index="<?php echo $data['id_schedule'] ?>" data-status="delete_schedule"
  ...
  <script>
    ...
    buttonsData.forEach((btnData) => {
      btnData.addEventListener('click', function () {
        ...
        const tanggalSchedule = btnData.parentElement.parentElement.querySelector('td:nth-child(3)').innerText;
        const waktuMulai = btnData.parentElement.parentElement.querySelector('td:nth-child(4)').innerText;
        const waktuSelesai = btnData.parentElement.parentElement.querySelector('td:nth-child(5)').innerText;
        const idCleaner = btnData.parentElement.parentElement.querySelector('td:nth-child(6)').dataset.idCleaner;

        const statusData = btnData.dataset.status;

        if (statusData === 'edit_schedule') {
          xhr.onreadystatechange = function () {
            if (xhr.status === 200 && xhr.readyState === 4) {
              const response = JSON.parse(xhr.responseText);

              modalTitle.textContent = 'Edit Data Schedule';
              modalContent.innerHTML = `<form method="POST" class="form">
            <div class="modal-body d-flex flex-column gap-3">
              <div class="d-flex flex-column gap-2">
                <label for="tanggal_Schedule">Tanggal Schedule</label>
                <input type="date" id="tanggal_Schedule" name="tanggal_Schedule" required value="${response['tanggal_schedule']}">
              </div>
              <div class="d-flex flex-column gap-2">
                <label for="waktu_mulai">Waktu Mulai</label>
                <input type="time" id="waktu_mulai" name="waktu_mulai" value="${response['waktu_mulai']}" step="any">
              </div>
              <div class="d-flex flex-column gap-2">
                <label for="waktu_selesai">Waktu Selesai</label>
                <input type="time" id="waktu_selesai" name="waktu_selesai" value="${response['waktu_selesai']}" step="any">
              </div>
              <div class="d-flex flex-column gap-2">
                <label for="cleaner">Waktu Selesai</label>
                <select id="cleaner" name="cleaner">
                ${response['cleaner'].map(cleaner => {
                return `${cleaner['id_user'] == idCleaner ?
                  `<option value="${cleaner['id_user']}" selected>${cleaner['username']}</option>` :
                  `<option value="${cleaner['id_user']}">${cleaner['username']}</option>`}`
              })}
                </select>
              </div>
          ...
          xhr.open('POST', `./crud/api.php?id_schedule=${indexData}&tanggal_schedule=${tanggalSchedule}&waktu_mulai=${waktuMulai}&waktu_selesai=${waktuSelesai}&action=${statusData}`, true);
          xhr.send();
        } else if (statusData === 'delete_schedule') {
    ...
    function editData(indexData) {
      const tanggalSchedule = document.getElementById('tanggal_Schedule');
      ...
      form.addEventListener('submit', function () {
        ...
        xhr.open('POST', `./crud/api.php?id_schedule=${indexData}&tanggal_schedule=${tanggalSchedule.value}&waktu_mulai=${waktuMulai.value}&waktu_selesai=${waktuSelesai.value}&cleaner=${cleaner.value}&action=submit_edit_schedule`, true);
        xhr.send();
      })
    }
    ...
    function deleteData(indexData) {
      ...
      btnDeleteData.addEventListener('click', function () {
        ...
        xhr.open('POST', `./crud/api.php?id_schedule=${[indexData]}&action=submit_delete_schedule`, true);
        xhr.send();
      })
    }
      ...
      btnDeleteData.addEventListener('click', function () {
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            console.log(response);
            if (response === 'success') {
              alert('Semua data schedule berhasil dihapus!');
            } else {
              alert('Semua data schedule gagal dihapus!');
            }
        ...
        xhr.open('POST', `./crud/api.php?action=delete_all_schedule`, true);
        xhr.send();
      })
    }
  </script>
